...
use nostr::{
    EventBuilder, Filter, Kind, Keys, PublicKey, RelayUrl, SingleLetterTag, Tag, TagKind,
    Timestamp,
};
...
#[derive(Debug, Parser)]
#[command(name = "fips-web-daemon", about = "Rust web/daemon runtime for the FIPS web console")]
struct Args {
...
fn default_advert_relays() -> Vec<String> {
    DEFAULT_ADVERT_RELAYS.iter().map(|v| v.to_string()).collect()
}
...
impl AppState {
    ...
    async fn publish_inbox_relays(&self) -> Result<()> {
        let tags = self
            ...
            .map(|relay| Tag::custom(TagKind::SingleLetter(SingleLetterTag::lowercase(nostr::Alphabet::R)), [relay.to_string()]))
            .collect::<Vec<_>>();
        let event = EventBuilder::new(Kind::InboxRelays, "")
            .tags(tags)
            .sign_with_keys(&self.keys)?;
        let _ = self.client.send_event_to(self.dm_relays.clone(), event).await?;
        Ok(())
    }
    ...
    async fn refresh_advert_cache(&self, wait_ms: u64) -> Result<()> {
        let filter = Filter::new()
            ...
            .since(Timestamp::from(Timestamp::now().as_u64().saturating_sub(3 * 24 * 60 * 60)));
        let events = self
            .client
            .fetch_events_from(self.advert_relays.clone(), filter, Duration::from_millis(wait_ms))
            .await?;
    ...
    async fn find_recipient_inbox_relays(&self, target_pubkey: PublicKey) -> Result<Vec<String>> {
        let filter = Filter::new()
            ...
            .since(Timestamp::from(Timestamp::now().as_u64().saturating_sub(30 * 24 * 60 * 60)));
    ...
    async fn send_shell_frame(&self, channel: &str, payload: Value, frame_type: &str) -> Result<()> {
        let active = self.active_session.read().await.clone().context("not connected")?;
...
async fn api_connect(
    ...
    let result: Result<Value> = async {
        let (target_npub, discovered_advert) = if npub.is_empty() {
            ...
            let advert = peers.into_iter().next().context("timed out waiting for any traversal advert after 60000ms")?;
