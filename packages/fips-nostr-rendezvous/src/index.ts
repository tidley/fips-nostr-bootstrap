...
function parseMappedAddress(buf) {
  if (buf.length < 8 || buf[1] !== 0x01) return null;
  return {
...
function parseStunBindingSuccess(packet, txnId) {
  ...
  if (packet.readUInt16BE(0) !== STUN_BINDING_SUCCESS) return null;
  if (packet.readUInt32BE(4) !== STUN_MAGIC_COOKIE) return null;
  if (!packet.subarray(8, 20).equals(txnId)) return null;
  ...
  while (offset + 4 <= maxOffset) {
    ...
    if (valueEnd > packet.length) break;
...
function parsePunchPacket(buf) {
  ...
  if (magic !== PUNCH_MAGIC && magic !== PUNCH_ACK_MAGIC) return null;
  return {
...
function parsePacket(buf) {
  ...
  if (binaryPunch) return { t: binaryPunch.type, h: binaryPunch.sessionHash };
  try {
...
function parseInboxRelayListEvent(evt) {
  if (!evt || evt.kind !== INBOX_RELAYS_KIND || !Array.isArray(evt.tags)) return null;
...
export class FipsNostrRendezvousNode extends EventEmitter {
  ...
  async _refreshTraversalObservation(opts = {}) {
    if (this.stunServers.length === 0) return this.stunObservation;
  ...
  async findRecipientInboxRelays(targetNpub, opts = {}) {
    if (!targetNpub) return opts.fallbackRelays ?? this.dmRelays;
    if (!this.pool?.subscribeMany) return opts.fallbackRelays ?? this.dmRelays;
    const decoded = nip19.decode(targetNpub);
    if (decoded.type !== 'npub') return opts.fallbackRelays ?? this.dmRelays;
    ...
    return await new Promise((resolve) => {
      ...
      sub = this.pool.subscribeMany(
        this.inboxLookupRelays,
        { kinds: [INBOX_RELAYS_KIND], authors: [targetPubkey], since: Math.floor(Date.now() / 1000) - 30 * 24 * 60 * 60 },
        {
          onevent: (evt) => {
            const parsed = parseInboxRelayListEvent(evt);
            if (!parsed || parsed.pubkey !== targetPubkey) return;
  ...
  _publishDeletion(eventIds, relays, reason) {
    if (!this.pool || !eventIds?.length) return;
    const evt = finalizeEvent({
